...
class Model(tf.keras.Model):
    def __init__(self, index_dict, max_dict, labels_dict):
        ...
        #this is the number of different possible events (outcomes) per at bat situation (ex: out, home run, single, double, etc.)
        #Thus, there are 19 different play outcomes in our dataset 
    ...
    def call(self, inputs):
        ...
        #previous lines were working with categories that were in the form of ints to perform lookups and do one-hotting
        #However, now we must cast the inputs to floats in order to perform concatenation 
        inputs = tf.cast(inputs, dtype=tf.float32)


        #grabbing data from corresponding columns of paramters and expanding dims to make them compatible for concatenation 
...
def train(model, train_inputs, train_labels):
    ...
    for x in range(0, train_inputs.shape[0] , model.batch_size): 
        ...
        batchinputs = train_inputs[x:x+model.batch_size, :]
        batchlabels = train_labels[x:x+model.batch_size, :]
        
        with tf.GradientTape() as tape: 
            probs = model.call(batchinputs)
            l = model.loss(probs, batchlabels)
...
def test(model, test_inputs, test_labels):
    ...
    for x in range(0, test_inputs.shape[0] , model.batch_size): 
        ...
        batchinputs = test_inputs[x:x+model.batch_size, :]
        batchlabels = test_labels[x:x+model.batch_size, :]
